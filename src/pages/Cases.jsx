...
export default function Cases() {
  ...
  return (
    <div>
      ...
      <Modal open={modal.open} onClose={() => setModal({ open: false, data: null })} title="案件詳細" maxWidth={720}
        footer={<><button className="btn btn-ghost" onClick={() => setModal({ open: false, data: null })}>關閉</button><button className="btn btn-danger" onClick={deleteCase}>刪除</button></>}>
        {modal.open && <>
