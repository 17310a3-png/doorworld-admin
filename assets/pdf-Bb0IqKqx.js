import{s as C}from"./supabase-EwPXOdIy.js";const s={nameZh:"門的世界 DOORWORLD",fullNameZh:"展億室內開發有限公司",taxId:"60667469",addr:"新北市五股區成泰路一段130-3號",phone:"02-2292-0366",email:"doorworld168@gmail.com"},R={single:"單開門",mother:"子母門",double:"雙開門",fire:"防火單門",room:"房間門",bathroom:"衛浴門",sliding:"橫拉門"};function l(t){return t?"NT$ "+Number(t).toLocaleString():"—"}function M(t){return(t?new Date(t):new Date).toLocaleDateString("zh-TW",{year:"numeric",month:"long",day:"numeric"})}function i(t){return String(t??"").replace(/[<>&"']/g,o=>({"<":"&lt;",">":"&gt;","&":"&amp;",'"':"&quot;","'":"&#39;"})[o])}async function U(t){if(!t){alert("找不到案件資料");return}const o=t.formal_quote_data||{};let g="";if(t.product_code)try{const a=await C(`products?full_code=eq.${encodeURIComponent(t.product_code)}&select=thumbnail_url,image_url`);g=a&&a[0]&&(a[0].thumbnail_url||a[0].image_url)||""}catch{}const m={};try{(await C("accessories?select=name,image_url&is_active=eq.true&image_url=not.is.null")||[]).forEach(e=>{e.image_url&&(m[e.name]=e.image_url)})}catch{}const O=o.fire_type==="f60a"?"f60A防火":o.fire_type==="f60a_smoke"?"f60A遮煙門":t.is_fireproof?"f60A防火":"不防火",j=R[t.door_type]||t.door_type||"單開門",A=M(t.official_quote_at||t.created_at),y=o.special_requirements||[],_=["拆舊","回收","佔框","濕式施工","乾式包框"].map(a=>(y.indexOf(a)>=0?"■":"□")+a).join(" ");let p="",w=0;t.addon_items&&t.addon_items.split(`
`).forEach(a=>{const e=a.trim();if(!e)return;let d=e.length-1;for(;d>=0&&(e[d]===" "||e.charCodeAt(d)>=48&&e.charCodeAt(d)<=57||e[d]===",");)d--;const n=d+1;if(n<e.length){const L=e.substring(n).trim().replace(/,/g,""),u=parseInt(L,10);if(!isNaN(u)){const P=e.substring(0,n).trim();w+=u,p+=`<tr><td class="tdl">${i(P)}</td><td class="tdv ra">${l(u)}</td></tr>`;return}}p+=`<tr><td class="tdl" colspan="2">${i(e)}</td></tr>`});let r=0;const c=t.quantity||1;if(t.official_note){const a=t.official_note.indexOf("門扇單價:");if(a!==-1){let e=a+5,d="";for(;e<t.official_note.length&&t.official_note.charCodeAt(e)>=48&&t.official_note.charCodeAt(e)<=57;)d+=t.official_note[e],e++;d&&(r=parseInt(d,10))}}!r&&t.official_price&&c>0&&(r=Math.round(t.official_price/c));const x=t.discount_rate||1,$=r*c,z=t.official_price||Math.round($*x),h=t.install_fee||0,f=t.total_with_tax||z+w+h,k=t.deposit_50||Math.round(f*.5),N=t.balance||f-k,q=o.width_mm||(t.actual_width_cm?t.actual_width_cm*10:""),S=o.height_mm||(t.actual_height_cm?t.actual_height_cm*10:""),v=o.accessories||[],T=v.length===0?"":`
    <div style="margin-top:6px">
      ...
      <table style="width:100%"><tr>
        ${v.map(a=>{const e=a.useUpgrade?a.upgrade:a.standard,d=a.useUpgrade?"#f9f6ec":"#fff",n=m[e]||m[a.standard]||"";return`<td style="border:1px solid #e2d5a0;padding:6px 8px;text-align:center;background:${d};vertical-align:top;width:${100/v.length}%">
            <div style="font-size:8px;font-weight:700;color:#c9a227;letter-spacing:1px;margin-bottom:3px">${i(a.label)}</div>
            ${n?`<img src="${i(n)}" alt="" style="width:60px;height:60px;object-fit:contain;border:1px solid #e2d5a0;border-radius:3px;margin-bottom:3px">`:'<div style="width:60px;height:60px;display:inline-block"></div>'}
            <div style="font-size:9px;font-weight:600;line-height:1.3">${i(e||"—")}</div>
    ...
    </div>`,D=`<!DOCTYPE html><html><head>
<meta charset="utf-8"><title>${i(t.formal_quote_no||"報價單")}</title>
...
<div class="page">
  <div class="hdr">
    <div class="hdr-l">
      ...
      <div class="company-info">
        甲方：${s.nameZh}（${s.fullNameZh}）　統編 ${s.taxId}<br>
        ${s.addr}　TEL: ${s.phone}　Email: ${s.email}
      </div>
    </div>
    <div class="hdr-r">
      <div class="qno">${i(t.formal_quote_no||t.order_no||t.case_no||"—")}</div>
      <div>日期：${A}</div>
      ${t.sales_person?`<div>業務：${i(t.sales_person)}</div>`:""}
      ${t.created_by?`<div>建單：${i(t.created_by)}</div>`:""}
    </div>
  </div>

  <div class="info-grid">
    <div class="info-cell"><div class="info-lbl">乙方 / 聯絡人</div><div class="info-val">${i(t.customer_name||t.contact_person||"—")}</div></div>
    <div class="info-cell"><div class="info-lbl">統編</div><div class="info-val">${i(t.tax_id||"—")}</div></div>
    <div class="info-cell"><div class="info-lbl">電話</div><div class="info-val">${i(t.customer_phone||"—")}</div></div>
    <div class="info-cell"><div class="info-lbl">樓層 / 電梯</div><div class="info-val">${o.has_elevator===!1?"無電梯":"有電梯"}</div></div>
    <div class="info-cell" style="grid-column:span 2"><div class="info-lbl">案場地址</div><div class="info-val">${i(t.case_address||t.customer_addr||"—")}</div></div>
  </div>
  ...
  <div class="sec">
    ...
    <div style="display:flex;gap:12px;align-items:flex-start">
      <table style="flex:1">
        <tr><td class="tdl">款式名稱</td><td class="tdv" style="font-weight:700">${i(t.product_code||"—")}</td>
            <td class="tdl">門的需求</td><td class="tdv">${i(j)}</td></tr>
        <tr><td class="tdl">防火需求</td><td class="tdv">${i(O)}</td>
            <td class="tdl">交貨時間</td><td class="tdv">${t.delivery_days||90} 日曆天</td></tr>
        <tr><td class="tdl">特殊需求</td><td class="tdv" colspan="3" style="font-size:9px">${y.length?_:"□無 "+_}</td></tr>
        <tr><td class="tdl">派送安裝</td><td class="tdv" colspan="3">${i(o.install_method||"甲方派送安裝")}</td></tr>
      </table>
      ${g?`<div style="flex-shrink:0;border:1px solid #e2d5a0;background:#fdfcf7;border-radius:4px;padding:6px;text-align:center"><img src="${i(g)}" alt="" style="width:150px;height:150px;object-fit:contain"><div style="font-size:7px;color:#999;margin-top:3px">效果圖僅供參考</div></div>`:""}
    </div>
  </div>
  ...
  <div class="sec">
    ...
    <table class="prod-tbl">
      ...
      <tr>
        <td>${q?Math.round(q/10)+" cm":"—"}</td>
        <td>${S?Math.round(S/10)+" cm":"—"}</td>
        <td>${i(o.frame_thickness||"—")}</td>
        <td>${i(o.panel_thickness||"—")}</td>
        <td>${i(o.door_direction||"—")}</td>
        <td>${c}</td>
        <td>${i(o.art_frame||"無")}</td>
        <td>${i(o.delivery_type||"框扇同時")}</td>
        <td>${i(o.door_color||"—")}</td>
        <td>${i(o.lock_style||"—")}</td>
        <td style="font-weight:700">${l(r)}</td>
      </tr>
    </table>
    ${T}
    ${t.note?`<div style="margin-top:4px;font-size:9px;color:#555;padding:4px 8px;background:#f9f6ec;border:1px solid #e2d5a0">備註：${i(t.note)}</div>`:""}
  </div>

  <div class="sec">
    <div class="stitle">訂單金額</div>
    <table>
      <tr><td class="tdl">門扇費用 (${c}樘)</td><td class="tdv ra">${l($)}</td></tr>
      ${x<1?`<tr><td class="tdl">折扣 (${Math.round(x*100)}%)</td><td class="tdv ra">${l(z)}</td></tr>`:""}
      ${h?`<tr><td class="tdl">安裝費</td><td class="tdv ra">${l(h)}</td></tr>`:""}
      ${p?`<tr><td class="tdl" colspan="2" style="background:#1a1a1a;color:#c9a227;font-size:8px;font-weight:700;letter-spacing:2px;text-align:left">附加施工費明細</td></tr>${p}`:""}
    </table>
    <div class="total-box">
      <div><div class="total-lbl">含稅總價</div></div>
      <div class="total-amt">${l(f)}</div>
    </div>
    <div class="pay-grid">
      <div class="pay-cell"><div class="pay-lbl">訂金 50%</div><div class="pay-val">${l(k)}</div></div>
      <div class="pay-cell"><div class="pay-lbl">尾款</div><div class="pay-val">${l(N)}</div></div>
      <div class="pay-cell"><div class="pay-lbl">含稅總價</div><div class="pay-val" style="color:#c9a227">${l(f)}</div></div>
    </div>
  </div>
  ...
  <div class="foot">
    <div>
      <div style="font-size:12px;font-weight:900;color:#1a1a1a;margin-bottom:3px">${s.nameZh}</div>
...
</body></html>`,b=window.open("","_blank","width=820,height=1160");if(!b){alert("請允許彈出視窗以列印 PDF");return}b.document.write(D),b.document.close()}export{U as p};
